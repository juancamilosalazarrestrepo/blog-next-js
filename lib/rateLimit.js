...
export async function checkRateLimit(req, identifierSuffix = "") {
  ...
  // Si Upstash no responde (caída, credenciales vencidas, sin red) permitimos
  // la petición: el rate limiting es una salvaguarda, no una puerta. Tumbar el
  // formulario con un 500 costaría leads reales por proteger de algo que no
  // está pasando.
  try {
    ...
    console.error("[rateLimit] Upstash no disponible, se permite la petición:", error);
